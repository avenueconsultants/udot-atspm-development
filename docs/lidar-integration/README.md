...
was validated above, but both are open before calling the branch fully CI-green.
**WP2 (Edge REST downloader client) and WP3 (object-events decoder) are implemented and
component-tested; live pilot-box validation remains** — see doc 08's progress notes. WP4
(scheduled end-to-end ingestion and configuration) is the next coding/operations package. The Edge API
...
| 2026-09-10 | Measured raw `object_events` retention on box `10.235.13.48` ≈ **9–12 days**. → ATSPM is the system of record; 5-min poll cadence; bounded manual backfill for the prototype; stored-event watermark before fleet rollout; "device N days behind" alert (WP8). |
...
| 2026-09-17 | **WP2 implemented and component-tested** — added `TransportProtocols.OusterBlueCityEdge` and `OusterBlueCityEdgeDownloaderClient` with OAuth client credentials, proactive/401 token refresh, configurable local-time window chunking, all-page `object_events` aggregation, TLS thumbprint or explicit allow-untrusted policy, redirect/cross-host protection, and a combined JSON temp-file envelope. Stub-backed tests cover the test-first 20-minute two-page/forced-401 flow, default endpoint derivation, chunking, and host rejection. Remaining WP2 acceptance: opt-in validation against `10.235.13.48` over VPN with rotated credentials. WP3 is the next coding package. |
| 2026-09-18 | **WP2 review corrections** — Claude's targeted review found four acceptance gaps. The client now reads the box timezone from authenticated `/config` when it is not configured, computes windows from UTC in that timezone, sends explicit-offset RFC 3339 timestamps, rejects zero `LoggingOffset`, normalizes host-root/API-root `BaseUrl` forms, bounds total minutes/chunks/pages, restricts resources to the Analytics API prefix, validates endpoint ports and malformed URLs, and keeps the injected test constructor out of DI selection. Security-handler tests now cover redirect blocking and certificate-pin precedence. Docs now state the prototype's initial backfill is a bounded manual run; a stored-event watermark is required before fleet rollout. |
| 2026-09-19 | **WP3 implemented** — added the auto-discovered `OusterBlueCityObjectEventsDecoder` for WP2's combined envelope. It maps every doc 03 field, strips offsets without UTC conversion, validates units, trims zone names, maps canonical/raw classifications, deduplicates on `LidarZoneEvent` equality, ignores beta fields, and wraps malformed input. Fixture tests include the sanitized doc 07 record, overlap, fall-back DST, empty/malformed input, unknown fields, and shared-importer range acceptance. |
