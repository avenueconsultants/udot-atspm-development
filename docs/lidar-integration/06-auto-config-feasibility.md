...
| Item | Resolution |
| --- | --- |
| **Phase number** | Inherited from the matched/《existing》`Approach`. Only "needs review" when a brand-new approach is created (rare — new detection on an existing signal still lands on an existing approach). **Verified safe to stage (2026-09-17, confirmed by spike doc 17 §7):** `Approach.ProtectedPhaseNumber` is a non-nullable `int` — but `0` is already ATSPM's established "no protected phase yet" sentinel (used by `LeftTurnGapReport`/`ApproachVolumeReportService`/`PhaseService`). Spike additionally confirmed **no DTO, service, EF, or inspected UI validator blocks or coerces a written `0`** (`ApproachDto`/`Approach` are unconstrained ints; `ApproachService` copies the value unchanged; the WebUI's `EditApproach.tsx` rejects null/blank, not `0`) — so staging `ProtectedPhaseNumber = 0` is safe at the data layer. **But this is a source-level finding, not a guarantee** — the needs-review/apply gate (doc 08 WP6) that stops an unreviewed zero-phase approach from going live is application logic ATSPM doesn't have today; it must actually be built as part of WP6, not assumed to come from existing validation. |
