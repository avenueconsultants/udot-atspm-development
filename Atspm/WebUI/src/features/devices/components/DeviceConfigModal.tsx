      ...
      >
        <Alert severity="info" variant="outlined">
          <Typography
            ...
            component="div"
          >
            You can insert dynamic values for both the path and query inputs by
            enclosing them in square brackets with either{' '}
            <strong>Device</strong> or <strong>DateTime</strong>, followed by a
            colon and the property or format. For example,{' '}
                ...
                {formatGroup === null ? (
                  <>
                    {deviceOptions.map((opt) => (
                      <Typography
                        key={`Device-${opt}`}
                        onClick={() => handleSuggestionClick(`[Device:${opt}]`)}
                        sx={{
                          cursor: 'pointer',
                          p: 0.5,
                          textAlign: 'left',
