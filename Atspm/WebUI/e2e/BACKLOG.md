...
- [x] **A3. Timing and Actuation** (L) — the largest option panel (detector
      ...
      comes back.
- [x] **A4. Turning Movement Counts** (M) — renders a table, not a chart;
...
- [x] **A20. Measure defaults** (M) — `/MeasureOption` defaults fill the
      ...
      **Presets are not part of this page.** Nothing under
      ...
      stays with B7.
- [x] **A21. Chart toolbox** (M) — per-chart controls (zoom, y-axis default
      input, export) on a rendered chart; `IndividualChartControls`.
      `chart-toolbox.spec.ts`. The toolbox only appears for a measure with
      more than one chart or one that supports bin step lines
      (`chartFeatureFlags.ts`), which is why a lone Purdue Phase
...
- 2026-09-20 - A30 review fixes: keep Watchdog charts mounted during background count refreshes so reconnects preserve legend and unconfigured-issue selections; add a deferred-response browser regression. Strict API teardown now captures a page snapshot before closing and attaches it on test or guard failure, with coverage for both failure paths and a real failing-fixture probe. Gate: production build passed; 138/138 Chromium tests with no retries; 34 focused Jest tests; typecheck ratchet unchanged at 783, with no file/error-code increases; scoped lint and formatting passed. App fix a76ca39b; browser specs and diagnostics in the commit carrying this line.
